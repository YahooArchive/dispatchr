...
module.exports = function () {
    ...
    Dispatcher.prototype.dispatch = function (actionName, payload, callback) {
        var action = new Action(actionName, payload, callback);
        ...
        if (nextAction) {
            ...
            actionPromise.then(function (result) {
                ...
                if (nextAction.callback) {
                    setImmediate(nextAction.callback, null, result);
                }
                setImmediate(self.next.bind(self));
            })['catch'](function (err) {
                debug('finished with error ' + nextAction.name, err);
                self.currentAction = null;
                if (nextAction.callback) {
                    setImmediate(nextAction.callback, err);
                }
                setImmediate(self.next.bind(self));
            });
        }
    ...
    Dispatcher.prototype.handleAction = function (action) {
        var self = this,
            ...
            handlers = Dispatcher.handlers[name] || [],
            handlerMap = {};

        debug('handling ' + name, payload);
        // TODO: Store the handlers such that we don't have to loop over each store during dispatch
        handlers.forEach(function (store) {
            var storeInstance = self.getStore(store.name),
                storeHandler = storeInstance[store.handler];

            handlerMap[store.name] = storeHandler.bind(storeInstance);
        });
        return action.handle(handlerMap);
    };
    ...
    Dispatcher.prototype.waitFor = function (stores, callback) {
        ...
        stores = stores.map(function (store) {
            return Dispatcher.getStoreName(store);
        });

        var waitPromise = currentAction.waitFor(stores);
        if (callback) {
            waitPromise.then(function (result) {
